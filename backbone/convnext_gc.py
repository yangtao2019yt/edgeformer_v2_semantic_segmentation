...
class _ConvNeXt_GC(nn.Module):
    def __init__(self, in_chans=3,
                 ...
                 gcc_bs_indices=[3, 3, 6, 2], # notice 3 means no gcc for stage 1, 2
...
class GCHW_conv(nn.Module):
    ...
    def __init__(self, dim, global_kernel_size):
        super().__init__()
        self.dim = dim // 2
        self.gks = global_kernel_size-1 if global_kernel_size%2==0 else global_kernel_size
        self.padding = (self.gks-1) // 2
        self.gch_conv = nn.Conv2d(self.dim, self.dim, kernel_size=(self.gks, 1), padding=(self.padding, 0), groups=self.dim)
        self.gcw_conv = nn.Conv2d(self.dim, self.dim, kernel_size=(1, self.gks), padding=(0, self.padding), groups=self.dim)
